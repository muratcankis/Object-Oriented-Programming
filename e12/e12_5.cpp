...
int main()
{
   ...
   while(!ilist.empty())//for(int j=0; j<=size; j++)
   {
      cout << "Liste boyutu : "<< ilist.size()<< endl;   
      cout << "Liste bosmu : " << ilist.empty()<<endl;  
      cout << ilist.back() << endl;     // display item from front
      ilist.pop_back();                // pop item off front
   }

...
int main()
{
    ...
    Point *ptr2= new Point;			// 2 object are defined: point1 and point2

    ptr1->move(100,50);			// point1 moves to (100,50)
	ptr1->print();				// point1's coordinates to the screen
	ptr1->move(20,65);			// point1 moves to (20,65)
	ptr1->print();				// point1's coordinates to the screen
	if(ptr1->is_zero())			// is point1 on (0,0)?
		cout << "point1 is now on zero point(0,0)" << endl;
	else
		cout << "point1 is NOT on zero point(0,0)" << endl;
	ptr2->print();		
	ptr2->move(0,0);				// point2 moves to (0,0)
	ptr2->print();	
	if(ptr2->is_zero())			// is point2 on (0,0)?
